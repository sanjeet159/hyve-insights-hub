...
const renderMarkdown = (content: string) =>
  content.split("\n").map((line, i) => {
    ...
    if (t.startsWith("## "))
      return (
        <h2 key={i} className="mb-5 mt-12 font-heading text-2xl font-bold text-foreground relative">
    ...
    if (t.startsWith("> "))
      return (
        <blockquote key={i} className="my-8 border-l-[3px] border-primary bg-accent/50 py-5 pl-6 pr-5 text-foreground/80 italic rounded-r-xl text-[15px] leading-relaxed">
          {t.slice(2).replace(/\*/g, "")}
        </blockquote>
      );
    if (t.startsWith("---"))
    ...
    if (/^\d+\.\s/.test(t)) {
      ...
      return (
        <div key={i} className="mb-3 flex gap-3.5 leading-relaxed">
          <span className="mt-0.5 flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-primary/10 text-[11px] font-bold text-primary">{num}</span>
          <span className="text-foreground/75 text-[15.5px]"
            dangerouslySetInnerHTML={{ __html: t.replace(/^\d+\.\s/, "").replace(/\*\*(.*?)\*\*/g, '<strong class="font-semibold text-foreground">$1</strong>') }} />
        </div>
      );
    }
    return (
      <p key={i} className="mb-5 text-foreground/70 leading-[1.85] text-[15.5px]"
        dangerouslySetInnerHTML={{
          __html: t
            .replace(/\*\*(.*?)\*\*/g, '<strong class="font-semibold text-foreground">$1</strong>')
            .replace(/\[([^\]]+)\]\((https?:\/\/[^)]+)\)/g, '<a href="$2" target="_blank" rel="noopener noreferrer" class="font-medium text-primary underline decoration-primary/30 underline-offset-3 hover:decoration-primary transition-colors">$1</a>'),
        }} />
    );
  });
...
const BlogPost = () => {
  ...
  const handleOpenTab = () => window.open(window.location.href, "_blank", "noopener,noreferrer");
  const handleShare = () => {
    if (navigator.share) navigator.share({ title: post.title, url: window.location.href });
    else window.open(`https://twitter.com/intent/tweet?url=${encodeURIComponent(window.location.href)}&text=${encodeURIComponent(post.title)}`, "_blank");
  };
  ...
  const jsonLd = {
    ...
    publisher: { "@type": "Organization", name: "HYVE", url: "https://hyvefreelance.com" },
  };
  ...
  return (
    <div className="min-h-screen bg-background">
      <Helmet>
        ...
        <link rel="canonical" href={`https://blog.hyvefreelance.com/blog/${post.slug}`} />
        <meta property="og:type" content="article" />
        <meta property="og:title" content={post.metaTitle} />
        <meta property="og:description" content={post.metaDescription} />
        <meta property="og:image" content={post.image} />
        <meta property="og:url" content={`https://blog.hyvefreelance.com/blog/${post.slug}`} />
      ...
      <motion.div
        ...
        style={{ scaleX, background: "linear-gradient(90deg, hsl(40 88% 51%), hsl(35 95% 55%))" }}
      />
          ...
          <div className="mx-auto max-w-3xl -mt-28 relative z-10">
            ...
            >
              ...
              <div className="flex items-center gap-4 text-xs text-muted-foreground">
                ...
                <span className="hidden sm:flex items-center gap-1.5"><BookOpen className="h-3.5 w-3.5" />{post.category}</span>
              </div>
              <div className="ml-auto flex items-center gap-1.5">
                <motion.button whileHover={{ scale: 1.06 }} whileTap={{ scale: 0.94 }} onClick={handleOpenTab}
                  className="rounded-lg border border-border/50 bg-accent/40 p-2 text-muted-foreground transition-colors hover:bg-accent hover:text-foreground" title="Open in new tab">
                  <ExternalLink className="h-3.5 w-3.5" />
                </motion.button>
                <motion.button whileHover={{ scale: 1.06 }} whileTap={{ scale: 0.94 }} onClick={handleShare}
                  className="rounded-lg border border-border/50 bg-accent/40 p-2 text-muted-foreground transition-colors hover:bg-accent hover:text-foreground" title="Share">
                  <Share2 className="h-3.5 w-3.5" />
                </motion.button>
                <motion.button whileHover={{ scale: 1.06 }} whileTap={{ scale: 0.94 }} onClick={handleCopyLink}
                  className="rounded-lg border border-border/50 bg-accent/40 p-2 text-muted-foreground transition-colors hover:bg-accent hover:text-foreground" title={copied ? "Copied!" : "Copy link"}>
                  {copied
      ...
      {/* Post FAQs — only shows if post has faqs array */}
        ...
        <section className="border-t border-border/40 bg-accent/20 py-20">
          <div className="container mx-auto px-4">
            ...
            <div className="grid gap-8 sm:grid-cols-2 lg:grid-cols-3">
              {fallbackRelated.map((p, i) => <BlogCard post={p} index={i} key={p.id} />)}
            </div>
          </div>
        </section>
      )}
